...
import { HeadphonesIcon, MusicIcon, User } from "lucide-react"
...
const FriendsActivity = () => {
    ...
    useEffect(()=>{
        if(user) fetchUsers()    //if user is authenticated only fetch users 
    },[fetchUsers,user])
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      >{user.fullName}</span>
                      {isPlaying && <MusicIcon className="size-3.5 text-emerald-400 shrink-0"/>}
                    </div>
